    ...
    <c:forEach items="${requestScope.ITEMS}" var="product">
            <li class="span3" data-id="${product.id}" item-price="${product.price}" item-name="${product.description.name}" item-order="${product.sortOrder}">
                    <div class="product-box">                                        
                            ...
                            <h4><c:out value="${product.description.name}"/></h4></a>
                            <h3>
                                    <c:choose>
                            ...
                            <c:if test="${product.image!=null}"><a href="<c:url value="/shop/product/" /><c:out value="${product.description.friendlyUrl}"/>.html"><img src="<c:url value="${product.image.imageUrl}"/>"/></a></c:if>
                            <div class="bottom">
